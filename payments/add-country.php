...
$errors = [];
$old = ['code' => '', 'name' => '', 'currency_code' => 'AUD', 'tax_rate' => '0.00', 'shipping_fee' => '0.00', 'is_active' => 1];

// Fetch available currencies for dropdown
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($old['code'] === '' || strlen($old['code']) > 10) {
        $errors[] = 'Country code is required (max 10 characters).';
    }
    ...
    if (empty($errors)) {
        ...
        if ($stmt->fetch()) {
            $errors[] = 'A country with this code already exists.';
        }
    }

    if (empty($errors)) {
        $stmt = $pdo->prepare("INSERT INTO countries (code, name, currency_code, tax_rate, shipping_fee, is_active) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$old['code'], $old['name'], $old['currency_code'], $old['tax_rate'], $old['shipping_fee'], $old['is_active']]);
        setFlash('success', 'Country added successfully.');
...
<div class="mb-4">
    ...
    <h2 class="fw-bold"><i class="fas fa-plus-circle me-2" style="color:var(--gold);"></i>Add Country</h2>
    <p class="text-white-50">Add a new country with tax and shipping settings</p>
</div>
...
<div class="card-dark p-4" style="max-width:600px;">
    <form method="POST" action="">
        <div class="mb-3">
            ...
            <input type="text" name="code" id="code" class="form-control form-dark" value="<?= sanitize($old['code']) ?>" placeholder="e.g. AU, US, GB" maxlength="10" required>
        </div>
        <div class="mb-3">
            <label for="name" class="form-label">Country Name <span class="text-danger">*</span></label>
            <input type="text" name="name" id="name" class="form-control form-dark" value="<?= sanitize($old['name']) ?>" placeholder="e.g. Australia" maxlength="100" required>
        </div>
        <div class="mb-3">
        ...
        <button type="submit" class="btn btn-gold"><i class="fas fa-save me-1"></i>Save Country</button>
    </form>
</div>
