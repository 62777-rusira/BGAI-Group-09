...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Insert
    if (empty($errors)) {
        $stmt = $pdo->prepare("INSERT INTO products (category_id, name, slug, description, price_aud, stock, image, material, weight, is_featured, is_active)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$category_id, $name, $slug, $description, $price, $stock, $imageName, $material, $weight, $is_featured, $is_active]);
...
<div class="page-header d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2><i class="fas fa-plus me-2" style="color:var(--gold);"></i>Add Product</h2>
        <p class="text-white-50 mb-0">Create a new product listing</p>
    </div>
...
<div class="card-dark p-4">
    <form method="POST" enctype="multipart/form-data">
        <div class="row g-4">
            <div class="col-md-8">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" class="form-control form-dark" rows="5"><?= sanitize($_POST['description'] ?? '') ?></textarea>
                </div>
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label">Material</label>
                        <input type="text" name="material" class="form-control form-dark" placeholder="e.g. 18K Gold, Sterling Silver"
                               value="<?= sanitize($_POST['material'] ?? '') ?>">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Weight</label>
                        <input type="text" name="weight" class="form-control form-dark" placeholder="e.g. 5.2g"
            ...
            <div class="col-md-4">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="stock" class="form-control form-dark" min="0" required
                           value="<?= sanitize($_POST['stock'] ?? '0') ?>">
                </div>
                <div class="mb-3">
                    <label class="form-label">Product Image <span class="text-danger">*</span></label>
                    <input type="file" name="image" class="form-control form-dark" accept="image/*" required>
                </div>
        ...
        <div class="d-flex justify-content-end gap-2">
            ...
            <button type="submit" class="btn btn-gold"><i class="fas fa-save me-1"></i> Save Product</button>
        </div>
    </form>
</div>
